...
public class SCCTStuServiceImpl implements SCCTStuService {
    ...
    @Override
    public Page<SCCTStu> queryByPageByNameorStuId(String name, String stu_id, int pageNo, int pageSize) {
        Page<SCCTStu> page = new Page<>();
        page.setPageNum(pageNo);
        page.setPageSize(pageSize);

        Integer pageTotalCount = scctStuDao.queryPageTotalCountsByCourseName(name, stu_id);
        System.out.println(pageTotalCount);
        page.setPageTotalCount((pageTotalCount));
        ...
        List<SCCTStu> items = scctStuDao.querySCCTStuByPageByNameorStuId(name, stu_id, begin, pageSize);
        page.setItems(items);
        return page;
    }
}
