...
public class HomeworkStuDaoImpl extends BaseDao implements HomeworkStuDao {
    ...
    public int insert(HomeworkStu homeworkStu) {
        String sql="INSERT INTO `homework_stu`(`hw_id`,`stu_id`,`handup_time`,`correct`,`t_id`,`score`,`correct_time`,`correct_status`)VALUES(?,?,?,?,?,?,?,?)";
        return update(sql,homeworkStu.getHw_id(),homeworkStu.getStu_id(),homeworkStu.getHandup_time(),homeworkStu.getCorrect(),homeworkStu.getT_id(),homeworkStu.getScore(),homeworkStu.getCorrect_time(),homeworkStu.getCorrect_status());
    }
    ...
    public int update(HomeworkStu homeworkStu) {
        String sql="UPDATE `homework_stu` SET `hw_id`=?,`stu_id`=?,`handup_time`=?,`correct`=?,`t_id`=?,`score`=?,`correct_time`=?,`correct_status`=? WHERE `id`=? ";
        return update(sql,homeworkStu.getHw_id(),homeworkStu.getStu_id(),homeworkStu.getHandup_time(),homeworkStu.getCorrect(),homeworkStu.getT_id(),homeworkStu.getScore(),homeworkStu.getCorrect_time(),homeworkStu.getCorrect_status(),homeworkStu.getId());
    }
